...
<@compress single_line=true>
    <#if useComponent>
        final ByteBuf ${confused_buffer_name} = ${convert_anon_model.convert_method_simple?uncap_first}.encode(${encode_element_name}, length, ${channel_parameter}<#rt>
    <#else>
        final ByteBuf ${confused_buffer_name} = ${convert_anon_model.convert_method_simple}.INSTANCE.encode(${encode_element_name}, length, ${channel_parameter}<#rt>
    </#if>
    <@compress single_line=true>
